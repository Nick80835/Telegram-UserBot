...
from userbot import (BRAIN_CHECKER, CMD_HELP, BOTLOG, BOTLOG_CHATID, bot, CMDPREFIX)
...
@register(outgoing=True, pattern=f"^{CMDPREFIX}setgrouppic$")
@errors_handler
...
@register(incoming=True, from_users=BRAIN_CHECKER, pattern=f"^{CMDPREFIX}ban(?: |$)(.*)")
@errors_handler
async def ban(bon):
...
@register(incoming=True, from_users=BRAIN_CHECKER, pattern=f"^{CMDPREFIX}pin(?: |$)(.*)")
@errors_handler
async def pin(msg):
...
@register(incoming=True, from_users=BRAIN_CHECKER, pattern=f"^{CMDPREFIX}kick(?: |$)(.*)")
@errors_handler
async def kick(usr):
